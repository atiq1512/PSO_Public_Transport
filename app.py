...
distance = st.sidebar.number_input("Distance (km)", min_value=0.0, max_value=100.0, value=10.0)
fare = st.sidebar.number_input("Fare (₹)", min_value=0.0, max_value=200.0, value=30.0)
cost = st.sidebar.number_input("Cost per Passenger (₹)", min_value=0.0, max_value=100.0, value=15.0)
...
y_pred = max(0, y_pred.item())  # safe scalar, no negatives
...
y_max = max(0, y_max.item())  # safe scalar
...
col1.metric(label="Estimated Passengers", value=f"{y_pred:,.2f}")
col2.metric(label="Max Predicted Passengers (Demo)", value=f"{y_max:,.2f}")

# =====================================================
# Feature Contribution & Sensitivity (Improved)
# =====================================================
...
# Scale contributions so that sum equals predicted passengers
...
st.markdown(
    """
    The PSO-based regression model predicts metro passenger demand by  
    optimizing regression parameters using swarm intelligence.  
    This approach supports **data-driven decision-making** for public transportation planning.
    """
)
